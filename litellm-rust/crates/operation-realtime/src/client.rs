...
//! the connection pool ([`crate::pool`]) can pre-establish an upstream,
...
use crate::transformation::RealtimeProviderConfig;
...
use crate::openai::OPENAI_REALTIME_CONFIG;
...
pub(crate) fn resolve_model(model: &str) -> Result<&str, Error> {
    match model.split_once('/') {
        Some(("openai", model)) if !model.is_empty() => Ok(model),
        Some(_) => Err(Error::InvalidProvider(
            "realtime supports OpenAI deployments only".to_string(),
        )),
        None if !model.is_empty() => Ok(model),
        None => Err(Error::InvalidRequest(
            "realtime model must not be empty".to_string(),
        )),
    }
}
...
pub(crate) async fn dial_upstream(
    ...
    let (upstream, _response) = connect_websocket(request)
        .await
        .map_err(|err| Error::Network(err.to_string()))?;
    Ok(upstream)
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn realtime<In, Out>(
    model: &str,
    api_key: Option<&str>,
    api_base: Option<&str>,
    idle_timeout: Option<Duration>,
...
/// Splice a pre-warmed upstream (taken from [`crate::pool`]) to the
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn realtime_warm<In, Out>(
    model: &str,
    handoff: crate::pool::WarmHandoff,
    idle_timeout: Option<Duration>,
    observe: impl FnMut(&RealtimeEvent) + Send,
    client_in: In,
    client_out: Out,
...
mod tests {
    ...
    #[test]
    fn resolves_openai_provider_model() {
        assert_eq!(
            resolve_model("openai/gpt-realtime").unwrap(),
            "gpt-realtime"
        );
        assert_eq!(resolve_model("gpt-realtime").unwrap(), "gpt-realtime");
        assert!(matches!(
    ...
    /// crypto provider installed, which is what the transport connector buys.
    #[tokio::test]
    ...
    ///   `cargo test -p litellm-operation-realtime realtime_invokes_openai -- --ignored --nocapture`
    #[tokio::test]
