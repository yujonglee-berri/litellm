...
pub(crate) struct WarmHandoff {
    pub(crate) tx: UpstreamTx,
    pub(crate) rx: UpstreamRx,
    pub(crate) session_created: RealtimeEvent,
}
...
impl RealtimePool {
    ...
    pub fn register(
        ...
        let model = crate::client::resolve_model(model)?;
        let key = upstream_key(model, api_key, api_base)?;
        self.register_key(key);
        Ok(())
    }

    fn register_key(&self, key: UpstreamKey) {
    ...
    /// fresh-dials.
    pub(crate) fn take(&self, key: &UpstreamKey) -> Option<WarmHandoff> {
        if !self.config.enabled() {
            return None;
        }
        loop {
    ...
    #[cfg(test)]
    fn warm_len(&self, key: &UpstreamKey) -> usize {
        self.warm
            .lock()
            .unwrap()
            .get(key)
    ...
    #[cfg(test)]
    fn backoff_failures(&self, key: &UpstreamKey) -> u32 {
        self.backoff
            .lock()
            .unwrap()
            .get(key)
    ...
    #[cfg(test)]
    async fn warm_now(&self, key: &UpstreamKey) {
        let needed = {
            let warm = self.warm.lock().unwrap();
            let have = warm.get(key).map(Vec::len).unwrap_or(0);
            self.config.target_size.saturating_sub(have)
...
pub(crate) fn upstream_key(
    model: &str,
    api_key: Option<&str>,
    api_base: Option<&str>,
) -> Result<UpstreamKey, Error> {
