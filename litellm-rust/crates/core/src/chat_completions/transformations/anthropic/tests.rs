...
fn transform(model: &str, msgs: Value, opts: Value) -> Value {
    ANTHROPIC_MESSAGES_TRANSFORMATION
...
fn transform_response(body: Value) -> Result<ChatCompletionsResponse, Error> {
    ANTHROPIC_MESSAGES_TRANSFORMATION.transform_response(ChatCompletionsTransformResponse {
...
fn reason(msgs: Value, opts: Value) -> Option<Unsupported> {
    ANTHROPIC_MESSAGES_TRANSFORMATION.unsupported_reason(&messages(msgs), &params(opts))
}
