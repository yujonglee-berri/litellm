...
use crate::chat_completions::types::{
    ...
    ChatCompletionsResponse, ChatCompletionsTransformResponse, ChatCompletionsUsage, ChatMessage,
    ChatMessageContent, ProviderChatRequestData,
};
...
/// core cannot see.
const ACCEPTED_MAPPED_PARAMS: &[&str] = &["maxTokens", "temperature", "topP", "stopSequences"];

pub(crate) const AWS_BEDROCK_RUNTIME_ENDPOINT: &str = "aws_bedrock_runtime_endpoint";
...
pub(crate) struct BedrockConverseTransformation;

pub(crate) const BEDROCK_CONVERSE_TRANSFORMATION: BedrockConverseTransformation =
...
fn converse_body(conversation: &Conversation, params: &Map<String, Value>) -> Value {
    ...
    let inference_config = Map::from_iter(ACCEPTED_MAPPED_PARAMS.iter().filter_map(|name| {
        params
            .get(*name)
...
impl OperationTransformation<ChatCompletionsOperation> for BedrockConverseTransformation {
...
impl ChatCompletionsTransformation for BedrockConverseTransformation {
...
impl RequestTransformation<ChatCompletionsOperation> for BedrockConverseTransformation {
...
impl ResponseTransformation<ChatCompletionsOperation> for BedrockConverseTransformation {
    ...
    fn transform_response(
        ...
        let ChatCompletionsTransformResponse { model, response } = input;
        let body = response
            .body
            .as_object()
