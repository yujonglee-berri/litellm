...
//! the connection pool ([`crate::realtime::pool`]) can pre-establish an upstream,
...
/// Splice a pre-warmed upstream (taken from [`crate::realtime::pool`]) to the
...
pub async fn realtime_warm<In, Out>(
    model: &str,
    handoff: crate::realtime::pool::WarmHandoff,
    idle_timeout: Option<Duration>,
    observe: impl FnMut(&RealtimeEvent) + Send,
    client_in: In,
    client_out: Out,
...
mod tests {
    ...
    #[test]
    fn provider_model_resolution_stays_inside_core() {
        assert_eq!(
    ...
    ///   `cargo test -p litellm-gateway-inference --features server realtime_invokes_openai -- --ignored --nocapture`
    #[tokio::test]
