...
use litellm_auth::{AuthError, InputSource, ResolvedCredential, SecretValue, Sourced};
...
mod tests {
    ...
    fn request_authority_requires_request_owned_client_secret_identity() {
        ...
        assert!(matches!(
            error,
            litellm_auth::AuthError::Configuration(
                litellm_auth::error::AuthConfigurationError::MixedAzureCredentialSources
            )
        ));
    }
    ...
    fn authority_is_restricted_to_an_https_origin() {
        for authority in [
            ...
            assert!(matches!(
                error,
                litellm_auth::AuthError::Configuration(
                    litellm_auth::error::AuthConfigurationError::InvalidAzureAuthority
                )
            ));
        }
    }
