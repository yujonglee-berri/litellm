...
//! the binary needs lives in the library (`litellm_gateway_inference`); `main` just
//! wires startup.
...
async fn main() {
    ...
    eprintln!("litellm-gateway-inference listening on {host}:{port}");
...
/// pre-warms it. Deployments whose model or key can't be resolved are skipped (they fresh-dial
