...
pub async fn run<In, Out>(
    router: &litellm_gateway_router::Router,
    model: &str,
    ...
    Out::Error: std::fmt::Display,
{
    ...
    let provider_model = litellm_core::responses::resolve_model(&params.model)?;
    ...
    let result = CallLifecycle::default()
        .run(context, (), instrumentation.as_ref(), |_| async move {
            litellm_core::responses::client::async_responses_websocket(
                provider_model,
