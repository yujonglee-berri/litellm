...
pub enum AuthError {
    ...
    #[error("{mechanism} credential acquisition failed: {message}")]
    TokenAcquisition {
...
pub enum AuthConfigurationError {
    ...
    #[error("credential query parameter already exists: {0}")]
    DuplicateQueryParameter(String),
    #[error("credential name cannot be empty")]
    EmptyCredentialName,
    #[error("basic-auth username cannot be empty")]
    EmptyBasicUsername,
...
pub enum AwsAuthError {
    ...
    #[error("request contains a header reserved for SigV4: {0}")]
    ComputedHeader(String),
    #[error("request body cannot be signed because it is streaming")]
    StreamingBody,
    #[error("request authentication produced an invalid header")]
    InvalidSignedHeader,
}
