...
use super::*;
use crate::{ResolvedCredential, TokenFuture, TokenProvider};
...
#[tokio::test]
async fn basic_auth_is_typed_and_marks_the_header_sensitive() {
    let auth = BasicAuth::new(
        SecretValue::new("client"),
        Some(SecretValue::new("secret")),
        ExistingHeaderBehavior::Reject,
    )
    .unwrap();

    let authenticated = auth.authenticate(request("token")).await.unwrap();
...
#[tokio::test]
async fn query_auth_encodes_and_rejects_duplicate_credentials() {
    let auth = QueryParamAuth::new(
        "key",
        SecretValue::new("a key&value"),
        ExistingHeaderBehavior::Reject,
    )
    .unwrap();
    let authenticated = auth.authenticate(request("models")).await.unwrap();
    assert_eq!(
        authenticated
            .url()
            .query_pairs()
        ...
        Some("a key&value".to_string())
    );

    let error = auth.authenticate(authenticated).await.unwrap_err();
    assert_eq!(
        error,
        AuthError::Configuration(AuthConfigurationError::DuplicateQueryParameter(
            "key".to_string()
        ))
    );
}
