use crate::Error;
use crate::wire::{ResponsesSessionEvent, ResponsesSessionTransformResult};
...
pub const OPENAI_RESPONSES_CONFIG: OpenAIResponsesConfig = OpenAIResponsesConfig;

impl OpenAIResponsesConfig {
    pub fn complete_websocket_url(&self, api_base: Option<&str>, model: &str) -> String {
        complete_websocket_url(api_base, model, true)
    }

    pub fn transform_client_event(
        &self,
        event: &ResponsesSessionEvent,
        model: &str,
    ) -> Result<ResponsesSessionTransformResult, Error> {
        Ok(ResponsesSessionTransformResult::passthrough(enforce_model(
            event, model,
        )))
    }

    pub fn transform_server_event(
        &self,
        event: &ResponsesSessionEvent,
        _model: &str,
    ) -> Result<ResponsesSessionTransformResult, Error> {
        Ok(ResponsesSessionTransformResult::passthrough(event.clone()))
    }
}
...
pub fn enforce_model(event: &ResponsesSessionEvent, model: &str) -> ResponsesSessionEvent {
...
mod tests {
    ...
    #[test]
    fn openai_config_enforces_model_on_client_create() {
        let event = event(serde_json::json!({"type":"response.create"}));
        let result = OPENAI_RESPONSES_CONFIG
            .transform_client_event(&event, "gpt-5")
